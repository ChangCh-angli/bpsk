...
//   码元       PN23(x^23+x^18+1,种子参数)
//   成形       RRC α=0.35,99 阶 Q1.14,转置直接型
//   输出       13-bit 有符号基带实信号,FS = ±4096
//
// amplitude <= 2797 时,PN23 全周期(8388607 码元)无削顶(实测峰值 4094)。
...
module bpsk_src_top #(
    ...
    sym_timing_nco #(.FTW(FTW)) u_nco (
        .clk(clk), .rst_n(rst_n), .ce(ce), .sym_strobe(sym_strobe)
    );
    ...
    rrc_pulse #(.TAP_FILE(TAP_FILE)) u_shaper (
        .clk       (clk),
        .rst_n     (rst_n),
        .ce        (ce),
        .imp_en    (sym_strobe),
