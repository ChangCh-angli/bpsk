/*
 * golden_bpsk.c -- BPSK 信号源的定点逐位 golden model
 *
 * 与 RTL 逐位等价,数据通路完全同构(务必与 rtl/ 保持同步):
 *   - 32-bit 相位累加器,FTW = 350469331,进位即码元边界
 *   - PN23: x^23+x^18+1,Fibonacci 左移,位 0 为当前码元
 *     (0 -> +A, 1 -> -A,对应 d = 1-2*bit)
 *   - 转置直接型 FIR:y[n] = acc[1] + x*TAP[0];acc[i] = acc[i+1] + x*TAP[i]
 *   - 输出:out = sat13( (y*A + 2^13) >>> 14 ),算术右移向下取整,
 *     饱和到 [-4096, 4095]
 *
 * 用法: golden_bpsk <n_samples> <amp> <seed> <ftw> <taps_dec> <out_hex>
 * 例:   golden_bpsk 2000000 2797 1 350469331 scripts/rrc_taps_dec.txt tb/golden_out.hex
 * 全周期峰值复核(约 1.028e8 采样,数十秒):
 *        golden_bpsk 102801655 2797 1 350469331 scripts/rrc_taps_dec.txt /tmp/full.hex
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int32_t tap[NTAPS];

int main(int argc, char **argv)
{
    if (argc != 7) {
        fprintf(stderr, "usage: %s <n_samples> <amp> <seed> <ftw> <taps_dec> <out_hex>\n",
                argv[0]);
        return 1;
    }
    ...
    if (!f || seed == 0) { fprintf(stderr, "bad tap file or zero seed\n"); return 1; }
    for (int i = 0; i < NTAPS; i++)
        if (fscanf(f, "%d", &tap[i]) != 1) { fprintf(stderr, "tap file short\n"); return 1; }
    fclose(f);
    ...
    for (long long s = 0; s < n_samp; s++) {
        ...
        int32_t y = accr[1] + x * tap[0];   /* x ∈ {-1,0,+1} */
        for (int i = 0; i < NTAPS; i++) {
            int32_t nxt = (i == NTAPS - 1) ? 0 : accr[i + 1];
            if (strobe)
                accr[i] = (x > 0) ? (nxt + tap[i]) : (nxt - tap[i]);
            else
                accr[i] = nxt;
        }
