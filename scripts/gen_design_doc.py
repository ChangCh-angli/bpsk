...
OUT = os.path.join(ROOT, "docs", "BPSK基带信号源设计说明书_V1.0.docx")
...
def main():
    ...
    r.font.size = Pt(20)
    r.bold = True
    for text in ("版本:V1.0", "日期:2026-09-11", "编写:izimu", "状态:已发布(验证通过)"):
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p.add_run(text)
    doc.add_page_break()
    ...
    add_table(doc,
              ...
              [["工作时钟", "100 MHz", "单时钟域,50 MHz 采样使能(每 2 拍一采样)"],
               ...
               ["脉冲成形", "RRC α=0.35", "99 阶(±4 码元),Q1.14 定点"],
               ["占用带宽", "≈ 5.51 MHz", "(1+α)×Rs"],
               ["输出位宽", "13-bit 有符号", "满量程 ±4096,量化 SNR ≈ 80 dB"],
               ["输出幅度", "默认 2797(−3.31 dBFS)", "全 PN23 周期实测零削顶,幅度端口可调"]])
    ...
    doc.add_paragraph(
        ...
        "为零),采用转置直接型后每个抽头级的乘法退化为“冲激到来时有条件地加减一个常数”,"
    ...
    doc.add_paragraph(
        "99 阶 Q1.14 抽头在 50 MHz 采样域按 t/T = n/12.2549 取样(峰值归一),由 "
        "scripts/gen_rrc_taps.py 生成并经 $readmemh 加载。核内 99 级 20-bit 累加器做"
    ...
    add_table(doc,
              ...
              [["clk", "in", "1", "100 MHz 工作时钟"],
               ...
               ["amplitude", "in", "13", "冲激幅度 LSB(≤2797 保证零削顶)"],
    ...
    code_par(doc,
             "Σ|tap| = 270 146 < 2¹⁹  →  累加器取 20-bit(理论最坏界内,含保护位)\n"
             "乘积位宽:|acc|max×amp_max = 524 287×4095 < 2³¹,输出级乘积 41-bit 容纳")
    doc.add_heading("5.3 幅度定标(为什么是 2797)", level=2)
    doc.add_paragraph(
        "RRC(α=0.35) 成形后 BPSK 波形的峰均比是固有属性。用定点 golden model 跑完整 "
        "PN23 周期(102 801 655 采样)实测:峰值 max|y| = 1.4639A(与种子无关——m-序列"
        "必含最坏局部图样,该峰值是确定值而非统计值),RMS = 0.9126A,峰均比 4.10 dB。"
    )
    add_table(doc,
              ["幅度 A 配置", "削顶情况", "输出 RMS"],
              [["2797(−3.31 dBFS)", "零削顶(峰值 4094 < FS 4095)", "−4.11 dBFS"],
               ["2896(−3.00 dBFS)", "约 0.4% 采样削顶,带外杂散抬升", "−3.81 dBFS"]],
              widths=[5.0, 7.0, 4.0])
    doc.add_paragraph(
        "结论:削顶换电平是坏交易(0.3 dB RMS 换取明显频谱杂散),默认幅度锁定 2797。"
        "幅度为运行时端口,若应用允许轻微削顶可自行上调。"
    )
    ...
    add_table(doc,
              ...
              [["逐位比对", "iverilog 13.0,2 000 000 采样", "PASS,0 失配"],
               ...
               ["全周期削顶", "golden,102 801 655 采样", "saturated=0(零削顶)"],
               ["NRZ 旁路", "100 000 采样", "±2797 阶梯波,节拍正确"],
